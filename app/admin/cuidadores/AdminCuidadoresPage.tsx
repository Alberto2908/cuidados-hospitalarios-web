...
import { fetchAdminCuidadores, type EstadoUsuario } from "@/lib/api/admin";
...
export default function AdminCuidadoresPage() {
  ...
  const { data: cuidadores = [], isLoading } = useQuery({
    queryKey: ["admin", "cuidadores"],
    queryFn: fetchAdminCuidadores,
  });
  ...
  const conValoracion = cuidadores.filter((c) => c.valoracionMedia != null);

  const totales = {
    total: cuidadores.length,
    activos: cuidadores.filter((c) => c.estado === "activo").length,
    suspendidos: cuidadores.filter((c) => c.estado === "suspendido").length,
    valoracionMedia:
      conValoracion.length === 0
        ? null
        : (conValoracion.reduce((acc, c) => acc + (c.valoracionMedia ?? 0), 0) / conValoracion.length).toFixed(1),
  };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard icon={<UserCheck className="h-5 w-5" />} label="Total cuidadores" display={String(totales.total)} color="text-foreground" />
        <StatCard icon={<UserCheck className="h-5 w-5" />} label="Activos" display={String(totales.activos)} color="text-emerald-600 dark:text-emerald-400" />
        <StatCard icon={<UserX className="h-5 w-5" />} label="Suspendidos" display={String(totales.suspendidos)} color="text-red-600 dark:text-red-400" />
        <StatCard
          icon={<Star className="h-5 w-5" />}
          label="Valoración media"
          display={totales.valoracionMedia == null ? "Sin datos" : `★ ${totales.valoracionMedia}`}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="px-4 py-3 text-center font-medium text-muted-foreground hidden md:table-cell">Tarifa</th>
                <th className="px-4 py-3 text-center font-medium text-muted-foreground hidden lg:table-cell">Cuidados realizados</th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                cuidadoresFiltrados.map((c) => (
                  <tr
                    key={c.id}
                    onClick={() => router.push(`/admin/cuidadores/${c.id}`)}
                  ...
                  >
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-foreground">{c.nombre} {c.apellidos}</p>
                    ...
                    <td className="px-4 py-3.5 text-center hidden md:table-cell">
                      <span className="font-medium text-foreground">
                        {c.tarifaHora != null ? `${c.tarifaHora} €/h` : "—"}
                      </span>
                    </td>
                    <td className="px-4 py-3.5 text-center hidden lg:table-cell">
                      <span className="font-medium text-foreground">{c.cuidadosRealizados}</span>
                    </td>
                    <td className="px-4 py-3.5 text-center hidden lg:table-cell">
                      {c.valoracionMedia != null ? (
                        <span className="font-medium text-amber-500 dark:text-amber-400">
                          ★ {c.valoracionMedia.toFixed(1)}
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3.5">
                      <EstadoUsuarioBadge estado={c.estado} />
                    </td>
                  </tr>
                ))
              )}
