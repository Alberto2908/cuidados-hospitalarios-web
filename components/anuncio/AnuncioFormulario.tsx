...
import { crearAnuncio, actualizarAnuncio } from "@/lib/api/anuncios";
import { ApiError } from "@/lib/api/client";

interface AnuncioFormularioProps {
  modo: "crear" | "editar";
  anuncioId?: string;
  valoresIniciales: CrearAnuncioFormValues;
}

/**
 * Mismo formulario para crear y modificar un anuncio (decision explicita:
 * el paciente/familiar puede equivocarse de dia, hora u hospital al
 * publicar). "editar" precarga valoresIniciales y llama a PATCH en vez de POST.
 */
export default function AnuncioFormulario({ modo, anuncioId, valoresIniciales }: AnuncioFormularioProps) {
  ...
  async function onSubmit(values: CrearAnuncioFormValues) {
    try {
      ...
      const anuncio = modo === "crear" ? await crearAnuncio(payload) : await actualizarAnuncio(anuncioId!, payload);

      sileo.success({
        title: modo === "crear" ? "Anuncio publicado" : "Anuncio actualizado",
        description:
          modo === "crear" ? "Los cuidadores ya pueden ver tu solicitud." : "Los cambios ya son visibles para los cuidadores.",
      });

      router.push(`/paciente/anuncio/${anuncio.id}`);
    } catch (error) {
      sileo.error({
        title: modo === "crear" ? "No se pudo publicar el anuncio" : "No se pudo actualizar el anuncio",
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div className="flex justify-center pt-2">
          ...
          >
            {isSubmitting
              ? modo === "crear" ? "Publicando…" : "Guardando…"
              : modo === "crear" ? "Publicar anuncio" : "Guardar cambios"}
          </Button>
        </div>
      </form>
    </div>
